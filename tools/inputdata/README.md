...
5. The generator writes the final outputs in a stable order so repeated runs produce the same files.
...
- `--state-out`, `--files-out`, `--standalone-files-out`, `--provenance-out`: output files to write

## Outputs

The generator writes four files to the paths passed on the command line:
...
- `files-out`: full-model inputdata URLs. This is the primary manifest consumed by the inputdata automation and other tooling that needs the full-model file list.
